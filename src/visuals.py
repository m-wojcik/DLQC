...
def animateBlochSphereTrajectory(trajx, trajy, trajz, nframes, title='bloch.mp4', writer='ffmpeg'):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.set(xlim3d=[-1.2,1.2], ylim3d=[-1.2,1.2], zlim3d=[-1.2,1.2])
    npoints = 100
...
def plotBlochSphereTrajectory(trajx, trajy, trajz, nframes, title='bloch.mp4', writer='ffmpeg'):
    ...
    line = ax.plot(trajx, trajy, trajz, lw=3)

    ar = ax.quiver(0, 0, 0, # <-- starting point of vector
            trajx[-1], trajy[-1], trajz[-1], # <-- directions of vector
            color = 'black', alpha = 1, lw = 3,
            arrow_length_ratio = 0.15)
